...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
	...
	fseek(fileToOpen, signitureOffset, SEEK_SET);
	for(int i=0; i<signitureSize; i++)
		fwrite(&null_byte, sizeof(char),1,fileToOpen);
	fclose(fileToOpen);
...
int main(int argc, char **argv) {
	while(1)
	{
		...
		else if(input_as_num==4)
		{
			...
			int size_of_virus;
			char filename[255];
			printf("Please enter killing file name: ");
			scanf("%s",filename);
			printf("Starting Byte: ");
			scanf("%d",&start);
			printf("Length: ");
            		scanf("%d",&size_of_virus);
			kill_virus(filename,start,size_of_virus);
			
		}
		else if(input_as_num==5)
		{
